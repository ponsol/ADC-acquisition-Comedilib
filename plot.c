...
int gnuplotopen()
{
            ...
            gp = popen("/usr/bin/gnuplot -persist", "w");
            //fprintf(gp,"set term vttek\n") ;
            //fprintf(gp,"set multiplot\n") ;
            fprintf(gp,"set key out below box\n") ;
...
//                 sprintf(key[j], "\"Chan:%d Zoom:%4.2f Offset:%g Mean: %f Rms: %f Tmean: %f  Trms: %f \"", 
 //                     achan, sfactor, doffset[j], pdat[achan].mean, 
  //                    pdat[achan].rms, pdat[achan].totmean, 
   //                   pdat[achan].totrms );
                 sprintf(key[j], "\"Chan:%2d Zoom:%4.2f Offset:%8.1g Mean: %8.1g Rms: %8.1g Tmean:%8.1g Trms:%8.1g\"", 
                      achan, sfactor, doffset[j], pdat[achan].mean, 
                      pdat[achan].rms, pdat[achan].totmean, pdat[achan].totrms);
//                 fprintf(stdout, "\"Chan:%2d Zoom:%4.2f Offset:%8.1g Mean: %8.1g Rms: %8.1g Tmean:%8.1g Trms:%8.1g\"", 
//                      achan, sfactor, doffset[j], pdat[achan].mean, 
//                      pdat[achan].rms, pdat[achan].totmean, pdat[achan].totrms);
                }
                 fflush(gp);
...
struct timeval  pretime[MAXCHAN] ;
     ...
     while(stopplot != STOPPLOT)
     {
         ...
         if (ret)
         {
            ...
            pnchan = getplotchannels(pchannels);
